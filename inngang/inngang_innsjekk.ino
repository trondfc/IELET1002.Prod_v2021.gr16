...
char cards[][5] = {
  {68, 236, 219, 49, 66},  // Astrid
  {34, 53, 137, 23, 137}, // Talha
  {214, 60, 225, 31, 20}, // Lauritz
  {211, 77, 23, 254, 119}, // Vilma
  {131, 19, 246, 26, 124}, // Trond
  {115, 111, 171, 26, 173} // Joakim
};
...
void loop() {
  ...
  if (rfid.isCard()) {
    ...
    int flag = compare_nested_array(cards, card_now);
      ...
      if(flag >= 0){Serial.println("ok");}
      Serial.println("-----------------------------------------");




    // om gyldig kort, print "Velkommen, "NAVN", you are a legend!!"
    if (flag >= 0) {
      Serial.print("Velkommen ");
      tft.print("Velkommen ");
      for (int i = 0; i < sizeof(name3); i++) {
        if ((*(names[flag] + i) != '\0') && (nameEndFlag == 0)) {
          Serial.print(*(names[flag] + i));
      ...
      Serial.println("   You are a legend!! ");
...
// gir beskjed til CoT om hvilken beboer som har sjekket inn
  if (flag == 0) {
    circusESP32.write(key1, 1, token);
  }
  if (flag == 1) {
    circusESP32.write(key2, 1, token);
  }
  if (flag == 2) {
    circusESP32.write(key3, 1, token);
  }
  if (flag == 3) {
    circusESP32.write(key4, 1, token);
  }
  if (flag == 4) {
    circusESP32.write(key5, 1, token);
  }
  if (flag == 5) {
    circusESP32.write(key5, 1, token);
...
//sammenligner liste med gyldige kort med det korte som er skannet, om det skannde kort eer et gyldig kort returneres true
